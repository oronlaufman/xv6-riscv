...
usertrapret(void)
{
  ...
  uint64 satp = MAKE_SATP(t->proc->pagetable);
  // printf("before trampoline by thread: %d\n", mythread()->tid);

  // jump to trampoline.S at the top of memory, which 
  // switches to the user page table, restores user registers,
  // and switches to user mode with sret.
  uint64 fn = TRAMPOLINE + (userret - trampoline);
  ((void (*)(uint64,uint64))fn)(TRAPFRAME + (uint64)(t->threadIndexInProc * sizeof(struct trapframe)), satp);
}
